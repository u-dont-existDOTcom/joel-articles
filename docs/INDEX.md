...
- `READER-QUESTION-AUDIT.md` — **optional negative-space diagnostic for broad long-form work.** Freeze opening-promise questions, preserve genuinely blind sequential reader checkpoints, then run a final hindsight coverage pass that distinguishes answered/late/partial/thin/unanswered/out-of-scope/rejected questions from actual article defects. The Romance pilot validated the reader-question method but rejected Obsidian Canvas as visually noisy; prefer compact Markdown/table/JSON output unless another visualization proves clearer.
