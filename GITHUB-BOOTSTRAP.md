...
## Owner-final Substack video boundary

Repository publishing/tooling authority must preserve this distinction:

- A standalone native Substack-uploaded video is nonportable through the clipboard helper. Preserve its exact raw island in archival HTML, exclude it from clipboard payloads, split the helper at its exact position, and require manual insertion using recorded identity and anchors.
- A Substack video-post preview/card is a different object type. Preserve its raw island in the archive, but keep its canonical post URL alone at the same position in the current payload segment; it does not force a split.
- Classify the enclosing `DigestPostEmbed` before descendant `<video>` markup. Keep YouTube separate.
