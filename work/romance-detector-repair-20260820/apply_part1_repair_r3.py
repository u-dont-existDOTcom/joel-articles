...
def apply_common(text: str) -> tuple[str, list[dict[str, object]]]:
    ops: list[dict[str, object]] = []
    for label, old, new in COMMON_REPLACEMENTS:
        text, op = replace_exact(text, label, old, new)
        ops.append(op)
    return text, ops
...
def main() -> int:
    ...
    master2, mops = apply_common(master)
    part1_2, pops = apply_common(part1)
    master2, op = replace_exact(master2, "slow-community-to-gandarussa-causal", SLOW_MASTER_OLD, SLOW_MASTER_NEW)
    mops.append(op)
    part1_2, op = replace_exact(part1_2, "slow-community-to-gandarussa-causal", SLOW_P1_OLD, SLOW_P1_NEW)
    pops.append(op)
    ...
    common_old_absent = all(old not in master2 for _, old, _ in COMMON_REPLACEMENTS)
    checks["r3_old_spans_absent"] = common_old_absent and SLOW_MASTER_OLD not in master2
