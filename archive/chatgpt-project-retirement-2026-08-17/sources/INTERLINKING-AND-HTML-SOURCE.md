...
- digest post previews, comments, Share, Subscribe, YouTube/video, paywalls, and unknown objects;
...
8. Prefer one complete payload. Do not split around a comment or another native object unless a real Substack destination test proves a separate step necessary.
...
| YouTube/native video | Preserve complete source block | Remove editor locks; retain native/iframe metadata, dimensions, URL, wrapper classes, and placement | Confirmed where tested |
...
| Rendered Substack comment card | Preserve rendered source only in archive | Replace the complete card with its canonical URL alone in its own paragraph at the same source position inside the one complete payload | Confirmed strategy; verify final reconstruction |
...
Do not remove all media, abandon one-paste automatically, split around comments without evidence, convert every object to a URL, infer unrelated failures, or treat a successful clipboard operation as successful publication transfer.
...
- the one payload covers the editor body exactly once after recorded conversions;
...
- comments are canonical URLs at their original positions and rendered comment HTML is absent;
...
Implement only the confirmed Opera local-file-to-editor path. Build a separate one-paste payload from the exact final archival HTML. Test the final helper in a blank Substack draft.
