...
SOURCE = WORK / "materialized-preservation-r10-part1"
SCRIPT = WORK / "apply_preservation_r11_part1.py"
...
OLD_PATIENT = "All three women told me at some point that they felt like my patient. Which is true"
NEW_PATIENT = "All three women eventually told me they felt like my patient, and I could see why."
...
class RomancePreservationR11Part1Tests(unittest.TestCase):
    ...
    def test_materializes_only_preservation_proved_r11_operations(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertTrue(checks["patient_cross_split_exact_known_human_match"])
            self.assertEqual(checks["patient_cross_split_sha256"], PATIENT_CROSS_SPLIT_SHA)
            ...
            for text in (master, p1):
                self.assertNotIn(SLOW_PREVIEW, text)
                self.assertNotIn(OLD_PATIENT, text)
                self.assertIn(NEW_PATIENT, text)
                self.assertIn("Something that developed between us had changed what her body could do with me.", text)
                self.assertIn("The sexual fit was real. It didn’t create the rest of the compatibility.", text)
            ...
            self.assertTrue(p2.startswith('Key at first asked me innocently, "Can you be my guru?"'))
    ...
    def test_source_hashes_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            bad_p1.write_text(
                (SOURCE / "candidate-part-1.txt").read_text(encoding="utf-8") + "\nmutant\n",
                encoding="utf-8",
            )
