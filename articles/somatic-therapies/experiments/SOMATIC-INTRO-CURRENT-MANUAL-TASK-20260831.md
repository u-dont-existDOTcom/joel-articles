# Somatic Introduction — current manual Chat writing task

Updated: 2026-08-31
Status: **CURRENT OWNER-DIRECTED TASK / non-authoritative until owner acceptance**

## Runtime

This task is performed by a ChatGPT reasoning/writing chat with Joel. Do not route Joel to Codex. Codex may be used only for bounded mechanical execution after Chat/Joel have made the editorial decision.

## Required fresh read

Before writing, recover current GitHub authority through `SKILL.md` and `CANONICAL-REPO-MAP.md`, then read the minimum current Somatic authority required by that read order.

For this task also read immediately before drafting:

1. `articles/somatic-therapies/experiments/SOMATIC-MANUAL-HUMANIZATION-WRITER-STATE-20260831.md`
2. `articles/somatic-therapies/experiments/SOMATIC-INTRO-ACTIVE-LESSON-CONTRACT-20260831.md`
3. `articles/somatic-therapies/experiments/SOMATIC-INTRO-EXTERNAL-PROGRESS-CONTROLLER-20260831.md`
4. current Pangram-humanization lesson index/working lessons required by `SKILL.md`

The active lesson contract is blocking. The external progress controller supersedes any interpretation of a hard reject as sufficient learning architecture: raw generations are disposable; only externally verified frontier advances may become owner-facing passes. A known lesson regression must be repaired before the candidate is shown to Joel.

## Current task

Write one genuinely human first section for the Somatic Therapies article.

The writer is not doing factual review, detector research, or a three-iteration autonomous experiment. Produce one candidate, show Joel, treat his response as the primary next signal, persist any new substantive correction, then reconstruct as needed.

## Information firewall

Do not request or use:

- the existing AI-written Introduction;
- previous rewrite candidates;
- Pangram-highlighted prose;
- unrelated Joel articles;
- Human-scoring donor passages;
- owner Human exemplars;
- Cancer, Romance, Community, transcripts, books, studies, clinical pages, or external prose as stylistic/sentence donors.

Do not invent personal trauma experiences, memories, anecdotes, chronology, symptoms, evidence, mechanisms, examples, authorities, quotations, terminology, or certainty.

## Semantic/function authority

The finished section must preserve all of these meanings/functions without turning them into visible checklist architecture:

- A traumatic event can be over while physical trauma reactions continue.
- A person can cognitively know they are safe while their body still reacts as though danger is present.
- With complex/developmental trauma, going directly to the deepest memory can be too much.
- Relevant readiness includes being able to remain present with what comes up rather than disappearing into it; being able to stop; and being basically oneself again afterward.
- If those capacities are absent, overwhelm should not be mistaken for successful **“deep processing.”**
- Depending on the actual problem, useful responses may include regulation; allowing accumulated survival activation some bodily way to discharge or resolve rather than relying only on thinking; or going directly to EMDR or another trauma-focused method when sufficient stability is already present.
- These are conditional responses to different situations, not mandatory sequential stages.
- Somatic state also matters for inner-child work.
- Someone may intellectually understand what a younger part needs but be too caught in the trauma/child-state to provide it.
- The body sometimes needs to become sufficiently manageable for an adult perspective to be available for relating to/reparenting the younger part.
- Preserve exactly: `http://innerchild.u-dont-exist.com/`
- Stop immediately before: `## Your Physical State Can Change What Therapy Does`

Attribution: do not convert general claims into Joel autobiography. First-person authorial judgment is allowed only where it means Joel's view or recommendation.

## Output for each attempt

Return only:

`# Introduction`

followed by the finished prose, and stop before the next section heading.

No analysis, alternatives, detector discussion, or notes unless Joel asks.

## Manual loop

After Joel responds:

1. treat his correction as new cognition, not merely line-edit feedback;
2. update the manual writer-state and active lesson contract when the correction is substantive/reusable;
3. update the external progress-controller state when the correction changes adjudication, learning retention, or promotion architecture;
4. reread the active lesson contract and external progress controller before the next attempt;
5. reconstruct rather than cosmetically edit when the problem is thought movement;
6. do not show a newer raw generation merely because it is newer; show one next candidate only when it is a verified frontier advance.
