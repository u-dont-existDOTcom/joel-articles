# Episode 008 R15 — coarse-to-fine preservation, plan, and cold-audit receipt
...
Candidate:
`articles/inner-child-therapy/experiments/EPISODE-008-R15-COARSE-TO-FINE-CANDIDATE-20260917.md`

Exact prose SHA-256, no terminal newline:
...
- The opening is not a manufactured relationship vignette. It is an authorial correction to the immediately preceding advice.
- Guilt/repair lives inside that correction instead of becoming a separate `both can be true` teaching paragraph.
- Gibson is support-level and appears after the live question already exists.
- The paragraph containing Gibson no longer carries the entire R13/R14 serial package of caveat -> recurrence -> framework -> motive -> revisability -> promise -> protection. Revisability/promise are moved into the next movement; protection is folded into the unresolved motive question (`Getting hurt again probably won't explain them any better`).
- The ending does not grant generic permission to end a conversation. It observes that the reader may already have said what mattered or said no, while disagreement remains, and leaves the internal continuation alive for the next exercise.

## 2. R14-P3 regression check

R14 P3 false-positive pattern:
`generalized setup -> engineered escalation -> consequence -> counselor permission/takeaway`.

R15 hidden outline:

- P1: direct correction of prior advice -> own-harm repair -> live question about the swallowed grievance -> guilt observation.
...
Rejected sequence:
`one-event caveat -> recurrence -> Gibson -> motive uncertainty -> revisability -> promise -> protection`.
...
Changed scope: outward-looking subsection after owner-final `Catch the Hook Before the Story Takes Over` and before `### Write It. Don't Send It Yet.`

Authorized delta: heading, realization, cadence, paragraphing, thought order, compression, and removal of model-only packaging. Claims, certainty, attribution/link, practical/protective functions, and downstream routing remain invariant.
...
| OUT-3 | One disagreement/time/boundary alone does not prove emotional immaturity; cannot/will-not/cannot-now uncertain; judgment revisable; promises not behavior change; protection need not wait for another injury | P2 allows one awful conversation/time/boundary and preserves `couldn't / wouldn't / just couldn't that day`; further injury is rejected as a way of solving motive. P3 allows changed behavior to change judgment and distinguishes promise from that behavior | PASS |
| OUT-4 | Guilt does not settle wrongdoing or prove manipulation; real repair and self-abandonment can coexist | P1: if mean, apologize; then recover what the speaker was trying to say; guilt can drop that point without manipulation | PASS |
| OUT-5 | Mutual understanding may never arrive; information/no/ending without prolonged defense can still occur | P3: reader may already have said what needed saying or said no; other person still thinks they're wrong; actual conversation is over | PASS |
...
- `maybe I was triggered -> all my fault` is a realization of the source's old-material-versus-present-boundary distinction, not a new factual claim.
- `If you were mean, apologize` realizes the protected genuine-repair function.
- `Good. Now...` is a reader-facing transition, not a new proposition.
- The one-awful-conversation/time/boundary possibilities are source-preserved qualifications.
- Gibson remains attributed only to repeated relational behavior versus motive-solving.
...
Heading promise: look outward as a correction to the preceding inward hook work.

Entry state: reader has just deferred the instantaneous `they're evil` judgment.

P1 live question: avoiding that verdict does not mean self-conviction; after legitimate apology, what was the reader actually trying to say?

P2 live question: perhaps nothing durable was wrong—but what if the original issue repeatedly disappears? Gibson supports watching recurrence while motive remains unresolved.

P3 live question: what if behavior later changes, or only a promise changes, and what if the other person never agrees? The external conversation may finish while internal speech continues.

Exit state: unresolved internal argument naturally requires the following private-draft practice.
...
Audit context: **context-contaminated / same drafting conversation**.

Detector/rationale blindness: **NO**. R14 history and its all-AI paragraph result are known. This audit is provisional and does not claim independent blindness.

Genre target: low-friction practical inner-child article in the communication mode calibrated by owner-final `Catch the Hook`.

### Strongest candidate 1

Span: `If you were mean, apologize. Good. Now what were you trying to say before the apology became the whole conversation?`
...
Span: P2 opening through the Gibson sentence.
...
Span: `If next time they actually show up differently, great—change your mind. A promise that next time will be different isn't the same thing yet.`
...
Technical/evidence density: **PASS**.
