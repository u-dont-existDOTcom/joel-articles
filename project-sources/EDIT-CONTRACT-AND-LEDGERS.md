...
A destination is not complete merely because the source article is correct. Check each derivative artifact. Rebuild the Substack transfer-ready payload and helper from the exact final archival HTML after every revision. Prefer one complete payload when no standalone native Substack-uploaded video exists. Each standalone native uploaded video is excluded from clipboard HTML and forces an ordered split plus a manual insertion step at its recorded identity and anchors. A Substack video-post preview/card is a separate type: preserve its exact archive island, replace it with its canonical post URL in the current payload segment, and do not split solely because it contains descendant video markup.
...
For an existing Substack article, inventory each image, digest preview, YouTube block, standalone native Substack-uploaded video, Substack video-post embed, Share object, Subscribe object, comment card, paywall, and unknown native object:
...
For the confirmed Opera/Substack path, locked invariants are conditional on object type. Without a standalone native uploaded video: downloaded local HTML; one visible `Copy Article` control; one complete payload wrapped in `<div dir="auto" class="body markup">`; only `contenteditable="false"` and `draggable="true"` removed from transferable native objects; immediate `ClipboardItem` with silent off-screen-contenteditable `execCommand("copy")` fallback; images, non-video digest previews, YouTube, Share, and Subscribe retained as rich HTML under their per-type status; rendered comments and Substack video-post preview/cards replaced by canonical URLs at their source positions; manual paywall placement only when a genuine source marker exists. With a standalone native uploaded video: the same rich-HTML invariants apply per ordered segment, native-video markup is absent from every clipboard payload, and each preserved native video has a manual insertion step between the corresponding segments using recorded identity and preceding/following anchors. Splits are not inferred for unrelated object failures.
