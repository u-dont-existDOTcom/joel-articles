...
### Positive owner-structure requirement

A negative gate is insufficient. Before the next Stage-1 generation attempt, read and apply:

`articles/somatic-therapies/experiments/SOMATIC-OWNER-POSITIVE-THOUGHT-SHAPE-MAP-20260907.md`

This is the missing owner-prose structural audit. It derives **thought relations**, not phrase recipes, from Joel's owner-controlled Somatic prose and natural research prose. The generator should choose the one owner-derived route that fits the local live pressure—for example `claim -> provenance/event -> consequence` or `practical uncertainty -> discriminating question`—rather than starting from the preservation inventory and trying to disguise it.

For the empty Somatic Experiencing lead specifically, practical pressure must precede bundled terminology. Do not require `titration`, `pendulation`, and `orientation` to appear in the same opening movement. Let one concrete relation become intelligible first; technical names enter only when the thought earns them. Score only newly model-generated bytes as generator evidence.
